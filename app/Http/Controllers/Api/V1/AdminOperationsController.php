...
class AdminOperationsController extends Controller
{
    ...
    private function serialize(string $section, $item): array
    {
        return match ($section) {
            ...
            'payments', 'commissions' => ['id' => $item->id, 'title' => '$'.$item->gross_amount.' MXN', 'subtitle' => ($item->client?->name ?? $item->professional?->user?->name ?? 'Pago').' · Ref. '.($item->external_reference ?? 'N/D'), 'status' => $item->status->value, 'fee' => (string) $item->platform_fee],
