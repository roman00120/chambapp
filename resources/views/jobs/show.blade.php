                            ...
                            <div class="quote-list">
                                @foreach ($jobRequest->quotes->sortByDesc('created_at') as $quote)
                                    ...
                                    <div class="quote-card {{ $quote->status === \App\Enums\QuoteStatus::ACCEPTED ? 'quote-card--accepted' : '' }}">
                                        ...
                                        @if ($isClient)
                                            @php $clientQuoteMoney = app(\App\Services\PaymentCalculationService::class)->calculateJob((string) $quote->amount); @endphp
                                            <div class="quote-earnings"><span>Precio base <strong>${{ $clientQuoteMoney->baseAmount }} MXN</strong></span><span>Cargo de servicio Chambapp ({{ $clientQuoteMoney->clientServiceFeePercent }}%) <strong>+${{ $clientQuoteMoney->clientServiceFee }} MXN</strong></span><span>Total <strong>${{ $clientQuoteMoney->customerTotal }} MXN</strong></span></div>
                                        @endif
                                        @if (! $isClient)
                                            @php $quoteMoney = app(\App\Services\PaymentCalculationService::class)->calculateJob((string) $quote->amount); @endphp
                                            <div class="quote-earnings"><span>Precio base <strong>${{ $quoteMoney->baseAmount }} MXN</strong></span><span>Comisión Chambapp ({{ $quoteMoney->professionalCommissionPercent }}%) <strong>-${{ $quoteMoney->professionalCommission }} MXN</strong></span><span>Monto estimado <strong>${{ $quoteMoney->professionalAmountBeforeExternalCosts }} MXN</strong></span></div>
                                            <p class="small text-muted mt-2 mb-0">Antes de impuestos, retenciones y costos externos aplicables.</p>
                                        @endif
                                        ...
                                        @if ($isClient && $quote->status === \App\Enums\QuoteStatus::PENDING && ! $quote->isExpired())
                                            <div class="quote-card__actions"><form method="POST" action="{{ route('job-quotes.accept', $quote) }}" data-confirm-form data-confirm-title="Aceptar cotización" data-confirm-message="Al aceptar, el trabajo quedará pendiente de pago dentro de Chambapp." data-confirm-submit="Aceptar cotización" data-confirm-submit-variant="primary">@csrf<x-ui.button type="submit" variant="primary"><i class="bi bi-check-lg" aria-hidden="true"></i> Aceptar</x-ui.button></form><form method="POST" action="{{ route('job-quotes.reject', $quote) }}" data-confirm-form data-confirm-title="Rechazar cotización" data-confirm-message="La cotización será rechazada. El profesional podrá enviar otra propuesta." data-confirm-submit="Rechazar cotización" data-confirm-submit-variant="danger">@csrf<div class="quote-reject-fields"><label class="visually-hidden" for="reject-reason-{{ $quote->id }}">Motivo del rechazo</label><select class="form-select form-select-sm" id="reject-reason-{{ $quote->id }}" name="reason" required><option value="">Motivo</option><option value="price_high">Precio alto</option><option value="changed_need">Cambió mi necesidad</option><option value="no_longer_needed">Ya no necesito el servicio</option><option value="other">Otro</option></select><label class="visually-hidden" for="reject-detail-{{ $quote->id }}">Detalle opcional</label><input class="form-control form-control-sm" id="reject-detail-{{ $quote->id }}" name="reason_detail" maxlength="140" placeholder="Detalle breve (opcional)"></div><x-ui.button variant="danger" type="submit">Rechazar</x-ui.button></form></div>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <x-ui.card class="job-sidebar-card mb-4" padding="lg">
                        ...
                        @if ($isClient)
                            @if ($jobRequest->status === \App\Enums\JobStatus::AWAITING_PAYMENT)
                                <div class="payment-placeholder"><span class="payment-placeholder__icon"><i class="bi bi-shield-lock" aria-hidden="true"></i></span><h3>{{ $jobRequest->service_id !== null ? 'Contratación directa' : 'Cotización aceptada' }}</h3><p>Total: <strong>{{ $jobRequest->formattedAgreedPrice() }}</strong></p>@if ($canPay)<a class="ui-button ui-button--primary w-100" href="{{ route('client.payments.summary', $jobRequest) }}">Pagar Chamba</a>@else<small>El profesional debe conectar Mercado Pago para habilitar el pago.</small>@endif</div>
                            @endif
                            @if ($jobRequest->status === \App\Enums\JobStatus::AWAITING_CONFIRMATION)<form method="POST" action="{{ route('job-requests.complete', $jobRequest) }}" data-disable-on-submit>@csrf<x-ui.button class="w-100" type="submit"><i class="bi bi-check2-circle" aria-hidden="true"></i> Confirmar finalización</x-ui.button></form>@endif
                            @if ($canCancel)<form class="mt-2" method="POST" action="{{ route('job-requests.cancel', $jobRequest) }}" data-disable-on-submit>@csrf<div class="mb-2"><label class="visually-hidden" for="cancel-reason-client">Motivo opcional</label><input class="form-control form-control-sm" id="cancel-reason-client" name="cancellation_reason" maxlength="255" placeholder="Motivo opcional"></div><x-ui.button class="w-100" variant="danger" type="submit">Cancelar trabajo</x-ui.button></form>@endif
                        @else
                            @if ($jobRequest->status === \App\Enums\JobStatus::PENDING && $jobRequest->service_id !== null)
                                <div class="d-flex flex-column gap-2 mb-3">
                                    <div class="alert alert-info py-2 px-3 small mb-2">
                                        <i class="bi bi-info-circle me-1"></i> Contratación directa por <strong>${{ number_format((float)($jobRequest->service?->price ?? 0), 2) }} MXN</strong>.
                                    </div>
                                    <form method="POST" action="{{ route('job-requests.accept-direct', $jobRequest) }}" data-disable-on-submit>
                                        @csrf
                                        <x-ui.button class="w-100" type="submit" variant="primary">
                                            <i class="bi bi-check-lg" aria-hidden="true"></i> Aceptar solicitud
                                        </x-ui.button>
                                    </form>
                                    <form method="POST" action="{{ route('job-requests.reject', $jobRequest) }}" data-disable-on-submit>
                                        @csrf
                                        <x-ui.button class="w-100" variant="danger" type="submit">
                                            <i class="bi bi-x-lg" aria-hidden="true"></i> Rechazar solicitud
                                        </x-ui.button>
                                    </form>
                                </div>
                            @elseif (in_array($jobRequest->status, [\App\Enums\JobStatus::PENDING, \App\Enums\JobStatus::ACCEPTED, \App\Enums\JobStatus::MATCHED, \App\Enums\JobStatus::AWAITING_QUOTE], true) && $jobRequest->service_id === null)
                                <form method="POST" action="{{ route('job-quotes.store', $jobRequest) }}" data-disable-on-submit>@csrf<div class="mb-3"><label class="form-label" for="quote-amount">Precio base propuesto (MXN)</label><input class="form-control" id="quote-amount" name="amount" type="number" min="0.01" max="99999999.99" step="0.01" value="{{ old('amount') }}" required><div class="form-text">Chambapp descontará 15% del precio base. El monto estimado es anterior a impuestos, retenciones y costos externos.</div></div><div class="mb-3"><label class="form-label" for="quote-description">Qué incluye</label><textarea class="form-control" id="quote-description" name="description" rows="3" maxlength="300" required placeholder="Ej. Incluye instalación y materiales básicos.">{{ old('description') }}</textarea><div class="form-text">Máximo 300 caracteres. No compartas datos de contacto.</div></div><x-ui.button class="w-100" type="submit"><i class="bi bi-send" aria-hidden="true"></i> Enviar cotización</x-ui.button></form>
                            @endif
                            @if ($jobRequest->status === \App\Enums\JobStatus::PAID)<form method="POST" action="{{ route('job-requests.on-the-way', $jobRequest) }}" data-disable-on-submit>@csrf<x-ui.button class="w-100" type="submit"><i class="bi bi-truck"></i> Avisar que voy en camino</x-ui.button></form>@endif
                            @if ($jobRequest->status === \App\Enums\JobStatus::ON_THE_WAY)<form method="POST" action="{{ route('job-requests.arrive', $jobRequest) }}" data-disable-on-submit>@csrf<x-ui.button class="w-100" type="submit"><i class="bi bi-geo-alt"></i> Marcar que llegué</x-ui.button></form>@endif
                            @if ($jobRequest->status === \App\Enums\JobStatus::ARRIVED)<form method="POST" action="{{ route('job-requests.start', $jobRequest) }}" data-disable-on-submit>@csrf<x-ui.button class="w-100" type="submit"><i class="bi bi-play-circle" aria-hidden="true"></i> Iniciar trabajo</x-ui.button></form>@endif
                            @if ($jobRequest->status === \App\Enums\JobStatus::IN_PROGRESS)<form method="POST" action="{{ route('job-requests.finish', $jobRequest) }}" data-disable-on-submit>@csrf<x-ui.button class="w-100" type="submit"><i class="bi bi-check2-circle" aria-hidden="true"></i> Marcar como terminado</x-ui.button></form>@endif
                            @if ($canCancel && $jobRequest->service_id === null)<form class="mt-2" method="POST" action="{{ route('job-requests.cancel', $jobRequest) }}" data-disable-on-submit>@csrf<div class="mb-2"><label class="visually-hidden" for="cancel-reason-pro">Motivo opcional</label><input class="form-control form-control-sm" id="cancel-reason-pro" name="cancellation_reason" maxlength="255" placeholder="Motivo opcional"></div><x-ui.button class="w-100" variant="danger" type="submit">Cancelar trabajo</x-ui.button></form>@endif
                        @endif
                    </x-ui.card>
                    <x-ui.card padding="lg"><h2 class="job-section-title mt-0">Historial del trabajo</h2><ol class="job-timeline"><li class="is-done"><span class="job-timeline__dot"></span><div><strong>{{ $jobRequest->service_id ? 'Contratación creada' : 'Solicitud enviada' }}</strong><small>{{ $jobRequest->created_at->locale('es')->translatedFormat('d M Y, g:i a') }}</small></div></li>@if ($jobRequest->service_id && $hasApprovedPayment)<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Pago aprobado</strong></div></li>@endif @if (!$jobRequest->service_id && $jobRequest->quotes->isNotEmpty())<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Cotización recibida</strong></div></li>@endif @if (!$jobRequest->service_id && $jobRequest->quotes->contains('status', \App\Enums\QuoteStatus::ACCEPTED))<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Cotización aceptada</strong></div></li><li class="is-done"><span class="job-timeline__dot"></span><div><strong>{{ $hasApprovedPayment ? 'Pago aprobado' : 'Pendiente de pago' }}</strong></div></li>@endif @if ($jobRequest->started_at)<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Trabajo iniciado</strong></div></li>@endif @if ($jobRequest->finished_at)<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Trabajo terminado</strong></div></li>@endif @if ($jobRequest->completed_at)<li class="is-done"><span class="job-timeline__dot"></span><div><strong>Trabajo completado</strong></div></li>@endif</ol></x-ui.card>
                </div>
            </div>
        </div>
    </section>
