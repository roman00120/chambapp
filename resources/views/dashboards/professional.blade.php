...
@section('content')
    <section class="dashboard-page">
        <div class="container">
            ...
            <x-ui.card class="mb-4" padding="lg"><div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3"><div><p class="eyebrow mb-2">Resumen económico</p><h2 class="h4 mb-2">Pagos aprobados</h2><p class="text-muted mb-0">{{ $paidJobsCount }} trabajos pagados · Bruto {{ app(\App\Services\PaymentCalculationService::class)->formatAmount((string) ($grossRevenue ?: '0.00')) }} · Comisión {{ app(\App\Services\PaymentCalculationService::class)->formatAmount((string) ($platformFees ?: '0.00')) }} · Profesional {{ app(\App\Services\PaymentCalculationService::class)->formatAmount((string) ($professionalRevenue ?: '0.00')) }}</p></div><div class="d-flex flex-column flex-sm-row gap-2"><x-ui.button href="{{ route('professional.earnings') }}" variant="outline">Ver ganancias</x-ui.button><x-ui.button href="{{ route('professional.payments.settings') }}" variant="link">Configurar Mercado Pago</x-ui.button></div></div></x-ui.card>
            ...
            <div class="row g-3 g-lg-4">
                ...
                <div class="col-12 col-lg-5">
                    <x-ui.card class="h-100" padding="lg">
                        <div class="d-flex justify-content-between align-items-center mb-3"><h2 class="h5 mb-0">Verificación</h2><x-ui.badge :variant="$profile->verificationBadgeVariant()" :label="$profile->verificationLabel()" /></div>
